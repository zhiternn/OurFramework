...
Mesh* MeshBuilder::GenerateCircle(const std::string &meshName, Color color, unsigned numSlice, float radius)
{
	...
	{
		yPos = sin(Math::DegreeToRadian(i * degreePerSlice)) * radius;
		xPos = cos(Math::DegreeToRadian(i * degreePerSlice)) * radius;
...
Mesh* MeshBuilder::GenerateCircleOutline(const std::string &meshName, Color color, unsigned numSlice, float length)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	...
	for (int i = 0; i < numSlice; ++i)
	{
		yPos = sin(Math::DegreeToRadian(i * degreePerSlice));
		xPos = cos(Math::DegreeToRadian(i * degreePerSlice));
