...
public:
	...
	Mesh* GenerateCircle(const std::string &meshName, Color color, unsigned numSlice = 36, float radius = 1.0f);
	Mesh* GenerateCircleOutline(const std::string &meshName, Color color, unsigned numSlice = 36, float radius = 1.0f);
	...
	Mesh* GenerateSprite(const std::string &meshName, unsigned row, unsigned col);
